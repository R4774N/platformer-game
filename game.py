...
class MyGame(arcade.Window):
    ...
    def on_key_press(self, key, modifiers):
        ...
        if key == arcade.key.S:
            ...
            self.player.change_x = MOVEMENT_SPEED
        elif key == arcade.key.W and self.physics_engine.can_jump(): # not self.player.is_jumping:
    ...
    def on_update(self, delta_time):
        ...
        if not self.game_over:
            ...
            collided_enemies = arcade.check_for_collision_with_list(self.player, self.enemy_list)

            if collided_enemies:
                if self.player.is_attacking:
                    self.enemy_list.remove(collided_enemies[0])
                else:
